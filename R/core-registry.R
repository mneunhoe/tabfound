...
#' @param handles_missing Logical. Does the network itself tolerate
#'   `NA`/`NaN` in the predictors? TabPFN encodes them explicitly and
#'   TabFM maps them to a sentinel; TabICL propagates them to NaN
#'   output, so [tabfound()] imputes by default for that backend.
...
register_backend <- function(name,
  ...
  spec <- list(
    ...
    handles_missing = isTRUE(handles_missing), parity = parity
  )
...
list_backends <- function() {
  ...
  if (length(nms) == 0L) {
    return(data.frame(name = character(), description = character(),
                      parity = character(), stringsAsFactors = FALSE))
  }
